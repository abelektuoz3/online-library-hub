...
function renderNavbar() {
    ...
    const publicLinks = [
        ...
        { name: 'Sign In', href: 'login.html', icon: 'fas fa-sign-in-alt', special: 'btn-outline' },
        { name: 'Join Free', href: 'register.html', icon: 'fas fa-user-plus', special: 'btn-primary' }
    ];
    ...
    const privateLinks = [
        ...
        { name: 'Contact', href: 'contact.html', icon: 'fas fa-envelope' },
        { name: 'About', href: 'about.html', icon: 'fas fa-info-circle' },
        { name: 'Logout', href: '#', icon: 'fas fa-sign-out-alt', special: 'logout' }
    ];
    ...
    linksToRender.forEach(link => {
        if (link.name === 'Logout') {
            ...
            if (isPageSubdir) {
                // We are inside /pages/ directory (e.g., catalog.html)
                // All internal pages are in the same directory
                if (link.name === 'Home') {
                    // Home is in root directory
                    href = '../index.html';
                } else {
                    // All other pages (Dashboard, Catalog, etc.) are in /pages/ directory
                    href = link.href;
                }
            } else {
                // We are in root directory (index.html)
                if (link.name === 'Home') {
                    href = 'index.html';
                } else {
                    // All other pages are in /pages/ directory
